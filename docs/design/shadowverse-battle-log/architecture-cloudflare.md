...
シャドウバースの対戦履歴を記録・分析するWebアプリケーション。フロントエンドはReact (SPA)、バックエンドはCloudflare Workers (サーバーレス)、データストレージはCloudflare R2 + D1 Database を使用した3層アーキテクチャ。
...
**使用用途**:
- **バックアップ**: 定期的なデータベースバックアップ（JSON形式）
- **エクスポート**: ユーザーがエクスポートしたファイルの一時保存
- **大容量ファイル**: 将来的なデッキ画像保存
...
| Cloudflare R2 | S3互換、無料枠10GB、Egress無料 | 🔵 *tech-stack.mdより* |
...
| オブジェクトストレージ | Blob Storage (5GB) | R2 (10GB、Egress無料) | Cloudflare優位 |
...
- **統合環境**: CloudflareはPages, Workers, D1, R2が統一されたプラットフォーム
