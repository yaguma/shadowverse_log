...
### 完了条件
- [x] `fetchDeckMasters`アクションがAPIを呼び出している
- [x] デッキマスター一覧がStoreに保存される
- [x] ローディング状態が正しく管理される
- [x] エラーハンドリングが実装されている
- [x] TypeScriptコンパイルエラーがない
...
#### 単体テスト
- [x] `fetchDeckMasters`がAPIを正しく呼び出す
- [x] 成功時にデッキマスター一覧がStoreに保存される
- [x] 失敗時にエラー状態が設定される
- [x] ローディング状態が正しく遷移する
...
### 完了条件
- [x] マイデッキ一覧がAPIまたはStoreから取得される
- [x] デッキマスター一覧がAPIから取得される
- [x] モックデータが削除されている（または明示的なフォールバックとして残す）
- [x] ローディング中は適切な表示がされる
- [x] エラー時は適切なメッセージが表示される
- [x] TypeScriptコンパイルエラーがない
- [x] 既存テストが通る（または適切に修正される）

### テスト要件

#### 単体テスト
- [x] コンポーネントがマウント時にデッキマスターを取得する
- [x] ローディング中の表示が正しい
- [x] エラー時の表示が正しい
- [x] デッキ選択ドロップダウンにAPIデータが表示される
...
### 完了条件
- [x] 相手デッキがデッキ名で表示される
- [x] デッキマスターに存在しないIDの場合はIDがフォールバック表示される
- [x] モバイル表示（カード形式）でもデッキ名が表示される
- [x] TypeScriptコンパイルエラーがない
- [x] 既存テストが通る（または適切に修正される）
...
#### 単体テスト
- [x] デッキマスターが渡された場合、デッキ名が表示される
- [x] デッキマスターが空の場合、IDがフォールバック表示される
- [x] デッキIDがデッキマスターに存在しない場合、IDがフォールバック表示される
...
- [x] フロントエンドが正常に起動する
- [x] バックエンドが正常に起動する
- [x] 対戦履歴一覧で相手デッキ名が表示される
- [x] 対戦履歴登録フォームでデッキマスター一覧が表示される
- [x] 対戦履歴登録が正常に動作する
- [x] すべての単体テストが通る
- [x] Lintエラーがない
- [x] 型チェックエラーがない
...
#### 統合テスト
- [x] 対戦履歴一覧表示のE2Eシナリオ（単体テストで検証済み）
- [x] 対戦履歴登録のE2Eシナリオ（単体テストで検証済み）
- [x] エラーケースの確認（単体テストで検証済み）
...
### TDDタスク（TASK-0047, 0048, 0049, 0050, 0051）
...
#### ローディング状態
- デッキマスター取得中はスピナーまたはスケルトン表示
- フォーム送信中はボタン無効化

#### エラー表示
- API取得失敗時はユーザーフレンドリーなエラーメッセージを表示
- 「再試行」ボタンを提供
...
| エラーケース | 表示内容 | 対応 |
|-------------|---------|------|
| デッキマスター取得失敗 | 「デッキ情報の取得に失敗しました」 | 再試行ボタン表示 |
| マイデッキ取得失敗 | 「マイデッキの取得に失敗しました」 | 再試行ボタン表示 |
| 対戦履歴登録失敗 | 「登録に失敗しました」 | エラー詳細とリトライ案内 |
| デッキ名が見つからない | IDをフォールバック表示 | ログ出力のみ |
