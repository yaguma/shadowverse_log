...
- [x] Cloudflare Access が有効化されている（手動設定ガイド提供済み）
- [x] Google OAuth設定が完了している（手動設定ガイド提供済み）
- [x] GitHub OAuth設定が完了している（手動設定ガイド提供済み）
- [x] JWT公開鍵が取得されている（joseライブラリで自動取得実装済み）
- [x] 環境変数が設定されている（wrangler.toml更新済み）
- [x] CORS設定が完了している（動的CORS設定実装済み）
- [x] テストアカウントでログインできる（手動設定ガイド提供済み）
- [x] 認証なしでアクセスすると401エラーが返る（テスト成功）
