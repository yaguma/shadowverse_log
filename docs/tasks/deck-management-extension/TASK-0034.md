# TASK-0034: 全機能統合E2Eテスト

## タスク情報

- **タスクID**: TASK-0034
- **タイプ**: TDD
- **推定工数**: 2時間
- **フェーズ**: Phase 6 - 対戦履歴UI改善
- **信頼性**: 🔵
- **依存タスク**: TASK-0033
- **後続タスク**: なし（最終タスク）
- **ステータス**: [ ] 未完了

---

## 概要

全機能を通した統合E2Eテストを作成する。デッキ種別追加→使用デッキ追加→対戦履歴登録→統計確認の一連のフローと、エッジケース（削除制約、空入力等）をテストする。

---

## 実装内容

### 1. 統合E2Eテストファイル作成

`packages/frontend/e2e/integration.spec.ts`:

```typescript
import { test, expect } from '@playwright/test';

test.describe('全機能統合テスト', () => {
  test.describe('フルフロー: デッキ追加→対戦記録→統計確認', () => {
    test('デッキ種別追加から統計確認までの完全フロー', async ({ page }) => {
      // 1. デッキ管理画面に移動
      await page.goto('/decks');
      await expect(page.locator('h1')).toHaveText('デッキ管理');

      // 2. デッキ種別を追加
      await page.click('button:has-text("デッキ種別を追加")');
      await expect(page.locator('[role="dialog"]')).toBeVisible();

      const testClassName = `テストクラス_${Date.now()}`;
      const testDeckName = `テストデッキ_${Date.now()}`;

      await page.fill('input[name="className"]', testClassName);
      await page.fill('input[name="deckName"]', testDeckName);
      await page.click('button:has-text("保存")');
      await expect(page.locator('[role="dialog"]')).not.toBeVisible();

      // デッキ種別が一覧に追加されたことを確認
      await expect(page.locator(`text=${testDeckName}`)).toBeVisible();

      // 3. 使用デッキタブに切り替え
      await page.click('button:has-text("使用デッキ")');
      await expect(page.locator('[data-testid="my-deck-list"]')).toBeVisible();

      // 4. 使用デッキを追加
      await page.click('button:has-text("使用デッキを追加")');
      await expect(page.locator('[role="dialog"]')).toBeVisible();

      const myDeckName = `マイデッキ_${Date.now()}`;

      // 先ほど追加したデッキ種別を選択
      await page.selectOption('select[name="deckId"]', { label: testDeckName });
      await page.fill('input[name="deckCode"]', 'test-deck-code-123');
      await page.fill('input[name="deckName"]', myDeckName);
      await page.click('button:has-text("保存")');
      await expect(page.locator('[role="dialog"]')).not.toBeVisible();

      // 使用デッキが一覧に追加されたことを確認
      await expect(page.locator(`text=${myDeckName}`)).toBeVisible();

      // 5. 統計画面に移動して対戦履歴を登録
      await page.click('nav >> text=統計');
      await expect(page).toHaveURL('/statistics');

      await page.click('button:has-text("対戦を記録")');
      await expect(page.locator('[role="dialog"]')).toBeVisible();

      // フォーム入力
      await page.selectOption('select[name="myDeckId"]', { label: myDeckName });
      await page.selectOption('select[name="opponentDeckId"]', { index: 1 });
      await page.selectOption('select[name="turn"]', '先攻');
      await page.selectOption('select[name="result"]', '勝ち');
      await page.click('button:has-text("保存")');
      await expect(page.locator('[role="dialog"]')).not.toBeVisible();

      // 6. 統計が更新されたことを確認
      await page.waitForResponse((response) =>
        response.url().includes('/api/statistics') && response.status() === 200
      );

      // 統計に使用デッキが表示されていることを確認
      await expect(page.locator(`text=${myDeckName}`).first()).toBeVisible();
    });
  });

  test.describe('エッジケース: 削除制約', () => {
    test('対戦履歴に使用されているデッキ種別は削除できない', async ({ page }) => {
      // 前提条件: 対戦履歴が存在するデッキ種別がある

      await page.goto('/decks');

      // 使用中のデッキ種別の削除ボタンをクリック
      // （実際の実装に応じて調整が必要）
      const deleteButton = page.locator('[data-testid="deck-master-row"]:has-text("使用中")').locator('button:has-text("削除")');

      if (await deleteButton.isVisible()) {
        await deleteButton.click();

        // 確認ダイアログが表示された場合
        const confirmDialog = page.locator('[role="alertdialog"]');
        if (await confirmDialog.isVisible()) {
          await page.click('button:has-text("削除")');
        }

        // エラーメッセージが表示されることを確認
        await expect(page.locator('text=削除できません')).toBeVisible();
      }
    });

    test('使用デッキに紐付いているデッキ種別は削除時に警告が表示される', async ({ page }) => {
      await page.goto('/decks');

      // テスト用のデッキ種別と使用デッキを作成
      // ... (セットアップ処理)

      // 削除時に警告メッセージが表示されることを確認
    });
  });

  test.describe('エッジケース: 空入力・バリデーション', () => {
    test('デッキ種別: 必須フィールドが空の場合は保存できない', async ({ page }) => {
      await page.goto('/decks');
      await page.click('button:has-text("デッキ種別を追加")');
      await expect(page.locator('[role="dialog"]')).toBeVisible();

      // 何も入力せずに保存ボタンをクリック
      await page.click('button:has-text("保存")');

      // HTMLバリデーションによりダイアログが閉じないことを確認
      await expect(page.locator('[role="dialog"]')).toBeVisible();

      // 必須フィールドにエラー表示があることを確認
      const classNameInput = page.locator('input[name="className"]');
      await expect(classNameInput).toHaveAttribute('required');
    });

    test('使用デッキ: デッキ種別未選択の場合は保存できない', async ({ page }) => {
      await page.goto('/decks');
      await page.click('button:has-text("使用デッキ")');
      await page.click('button:has-text("使用デッキを追加")');
      await expect(page.locator('[role="dialog"]')).toBeVisible();

      // デッキ種別を選択せずに保存
      await page.fill('input[name="deckCode"]', 'test-code');
      await page.fill('input[name="deckName"]', 'テスト');
      await page.click('button:has-text("保存")');

      // ダイアログが閉じないことを確認
      await expect(page.locator('[role="dialog"]')).toBeVisible();
    });

    test('対戦履歴: 使用デッキ・相手デッキ未選択の場合は保存できない', async ({ page }) => {
      await page.goto('/statistics');
      await page.click('button:has-text("対戦を記録")');
      await expect(page.locator('[role="dialog"]')).toBeVisible();

      // 何も選択せずに保存
      await page.click('button:has-text("保存")');

      // ダイアログが閉じないことを確認
      await expect(page.locator('[role="dialog"]')).toBeVisible();
    });
  });

  test.describe('エッジケース: データなし状態', () => {
    test('デッキ種別0件: 空状態メッセージが表示される', async ({ page }) => {
      // 注: このテストは実際のデータ状態に依存
      // モックAPIを使用するか、テスト用の状態をセットアップする必要あり
    });

    test('対戦履歴0件: 統計画面で適切なメッセージが表示される', async ({ page }) => {
      await page.goto('/statistics');

      // データがない場合のメッセージを確認
      // 実際のメッセージに応じて調整
      const emptyMessage = page.locator('text=統計データがありません');
      const recordButton = page.locator('button:has-text("最初の対戦を記録する")');

      // いずれかが表示されることを確認
      const isEmptyMessageVisible = await emptyMessage.isVisible().catch(() => false);
      const isRecordButtonVisible = await recordButton.isVisible().catch(() => false);

      if (!isEmptyMessageVisible && !isRecordButtonVisible) {
        // 統計データがある場合はスキップ
        test.skip();
      }
    });
  });

  test.describe('ナビゲーション連携', () => {
    test('全ページ間を正常に遷移できる', async ({ page }) => {
      // ホーム → 対戦履歴
      await page.goto('/');
      await page.click('nav >> text=対戦履歴');
      await expect(page).toHaveURL('/battle-logs');

      // 対戦履歴 → 統計
      await page.click('nav >> text=統計');
      await expect(page).toHaveURL('/statistics');

      // 統計 → デッキ管理
      await page.click('nav >> text=デッキ管理');
      await expect(page).toHaveURL('/decks');

      // デッキ管理 → ホーム
      await page.click('nav >> text=ホーム');
      await expect(page).toHaveURL('/');
    });

    test('各ページでデータが正しく表示される', async ({ page }) => {
      // 対戦履歴ページ
      await page.goto('/battle-logs');
      await expect(page.locator('h1')).toBeVisible();

      // 統計ページ
      await page.goto('/statistics');
      await expect(page.locator('h1')).toBeVisible();

      // デッキ管理ページ
      await page.goto('/decks');
      await expect(page.locator('h1')).toBeVisible();
    });
  });

  test.describe('シーズン連携', () => {
    test('統計画面でシーズンを切り替えても対戦履歴登録ダイアログに反映される', async ({ page }) => {
      await page.goto('/statistics');

      // シーズン一覧が読み込まれるまで待機
      await page.waitForSelector('#season-select option');

      // シーズンを選択
      const select = page.locator('#season-select');
      const options = select.locator('option');
      const optionCount = await options.count();

      if (optionCount > 1) {
        // 2番目のシーズンを選択
        await select.selectOption({ index: 1 });

        // 対戦履歴登録ダイアログを開く
        await page.click('button:has-text("対戦を記録")');
        await expect(page.locator('[role="dialog"]')).toBeVisible();

        // ダイアログ内のシーズン選択が連動していることを確認
        const dialogSeasonSelect = page.locator('[role="dialog"] select[name="season"]');
        const selectedSeason = await dialogSeasonSelect.inputValue();
        const expectedSeason = await options.nth(1).getAttribute('value');

        expect(selectedSeason).toBe(expectedSeason);
      }
    });
  });
});
```

### 2. テストデータ管理ユーティリティ

`packages/frontend/e2e/utils/test-data-manager.ts`:

```typescript
import { Page } from '@playwright/test';

/**
 * テストデータを作成するユーティリティ
 */
export class TestDataManager {
  constructor(private page: Page) {}

  /**
   * デッキ種別を作成
   */
  async createDeckMaster(className: string, deckName: string): Promise<string> {
    await this.page.goto('/decks');
    await this.page.click('button:has-text("デッキ種別を追加")');
    await this.page.fill('input[name="className"]', className);
    await this.page.fill('input[name="deckName"]', deckName);
    await this.page.click('button:has-text("保存")');
    await this.page.waitForSelector('[role="dialog"]', { state: 'hidden' });

    // 作成されたIDを返す（実装に応じて調整）
    return deckName;
  }

  /**
   * 使用デッキを作成
   */
  async createMyDeck(deckMasterName: string, deckCode: string, deckName: string): Promise<string> {
    await this.page.goto('/decks');
    await this.page.click('button:has-text("使用デッキ")');
    await this.page.click('button:has-text("使用デッキを追加")');
    await this.page.selectOption('select[name="deckId"]', { label: deckMasterName });
    await this.page.fill('input[name="deckCode"]', deckCode);
    await this.page.fill('input[name="deckName"]', deckName);
    await this.page.click('button:has-text("保存")');
    await this.page.waitForSelector('[role="dialog"]', { state: 'hidden' });

    return deckName;
  }

  /**
   * 対戦履歴を登録
   */
  async createBattleLog(
    myDeckName: string,
    opponentDeckIndex: number = 1,
    turn: '先攻' | '後攻' = '先攻',
    result: '勝ち' | '負け' = '勝ち'
  ): Promise<void> {
    await this.page.goto('/statistics');
    await this.page.click('button:has-text("対戦を記録")');
    await this.page.selectOption('select[name="myDeckId"]', { label: myDeckName });
    await this.page.selectOption('select[name="opponentDeckId"]', { index: opponentDeckIndex });
    await this.page.selectOption('select[name="turn"]', turn);
    await this.page.selectOption('select[name="result"]', result);
    await this.page.click('button:has-text("保存")');
    await this.page.waitForSelector('[role="dialog"]', { state: 'hidden' });
  }
}
```

### 3. 統合テスト用フィクスチャ

`packages/frontend/e2e/fixtures/integration-fixtures.ts`:

```typescript
import { test as base } from '@playwright/test';
import { TestDataManager } from '../utils/test-data-manager';

interface IntegrationFixtures {
  testDataManager: TestDataManager;
  uniqueId: string;
}

export const test = base.extend<IntegrationFixtures>({
  testDataManager: async ({ page }, use) => {
    const manager = new TestDataManager(page);
    await use(manager);
  },

  uniqueId: async ({}, use) => {
    const id = `test_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    await use(id);
  },
});

export { expect } from '@playwright/test';
```

---

## テスト要件

### E2Eテストケース一覧

| カテゴリ | テストケース | 説明 |
|---|---|---|
| フルフロー | 完全フロー | デッキ追加→対戦記録→統計確認 |
| 削除制約 | 使用中デッキ削除 | 対戦履歴に使用されているデッキは削除不可 |
| バリデーション | 必須フィールド空 | 保存ボタンが無効または警告表示 |
| データなし | 空状態 | 適切なメッセージ表示 |
| ナビゲーション | ページ遷移 | 全ページ間の遷移が正常 |
| シーズン連携 | シーズン選択反映 | 統計画面のシーズンがダイアログに反映 |

---

## 完了条件

- [ ] 統合E2Eテストファイルが作成されている
- [ ] フルフローテストが成功する
- [ ] エッジケーステストが成功する
- [ ] ナビゲーション連携テストが成功する
- [ ] すべてのE2Eテストが成功する

---

## 実行コマンド

```bash
# 統合E2Eテスト実行
pnpm exec playwright test e2e/integration.spec.ts

# すべてのE2Eテストを実行
pnpm exec playwright test

# 失敗したテストのみ再実行
pnpm exec playwright test --last-failed

# テストレポート表示
pnpm exec playwright show-report
```

---

## 検証手順

1. `packages/frontend/e2e/integration.spec.ts` が存在するか確認
2. `pnpm exec playwright test e2e/integration.spec.ts` でテストが成功するか確認
3. テストレポートを確認（`playwright-report/index.html`）
4. 全E2Eテストを実行して成功することを確認

---

## 関連ドキュメント

- [TASK-0033: 対戦履歴UI改善E2Eテスト](./TASK-0033.md)
- [デッキ管理機能拡張 技術設計書](../../design/deck-management-extension/)
- [デッキ管理機能拡張 要件定義書](../../spec/deck-management-extension-requirements.md)

---

## プロジェクト完了チェックリスト

このタスクが完了すると、デッキ管理機能拡張プロジェクトのすべてのタスクが完了となる。
以下のチェックリストで最終確認を行うこと:

### Phase 1〜3（既存）
- [ ] デッキマスターCRUD API
- [ ] マイデッキCRUD API
- [ ] デッキ関連コンポーネント

### Phase 4: デッキ管理画面統合
- [ ] TASK-0022: DeckManagePageタブ切り替え
- [ ] TASK-0023: ナビゲーション拡張
- [ ] TASK-0024: デッキ管理画面E2Eテスト

### Phase 5: 統計画面機能拡張
- [ ] TASK-0025: シーズン一覧取得API
- [ ] TASK-0026: シーズン別統計取得API
- [ ] TASK-0027: StatisticsStore拡張
- [ ] TASK-0028: シーズン選択UI
- [ ] TASK-0029: 対戦履歴登録ボタン
- [ ] TASK-0030: 統計画面E2Eテスト

### Phase 6: 対戦履歴UI改善
- [ ] TASK-0031: レイアウト改善
- [ ] TASK-0032: 相手デッキソート
- [ ] TASK-0033: UI改善E2Eテスト
- [ ] TASK-0034: 全機能統合E2Eテスト（本タスク）

### 最終確認
- [ ] 全テストスイートが成功する
- [ ] TypeScriptエラーがない
- [ ] Lintエラーがない
- [ ] ドキュメントが最新である
