# TASK-0018: MyDeckListコンポーネント実装

## タスク概要

- **タスクID**: TASK-0018
- **タイトル**: MyDeckListコンポーネント実装
- **タイプ**: TDD
- **推定工数**: 2時間
- **フェーズ**: Phase 3 - 使用デッキ管理機能
- **信頼性**: 🔵（interfaces.ts MyDeckListPropsより）
- **依存タスク**: TASK-0017
- **後続タスク**: TASK-0019
- **ステータス**: [ ] 未着手

---

## 要件

### 機能要件

- 使用デッキ一覧表示
- ローディング状態表示
- 空リスト時メッセージ表示（REQ-EXT-502）
- 追加ボタン配置

### 関連要件ID

- REQ-EXT-502: 空リスト時メッセージ表示

---

## 実装詳細

### 1. コンポーネントProps定義

```typescript
// packages/web/src/components/deck/types.ts

import type { MyDeck } from '@shadowverse-log/shared';

export interface MyDeckListProps {
  /** 使用デッキ一覧 */
  myDecks: MyDeck[];
  /** ローディング状態 */
  isLoading: boolean;
  /** エラーメッセージ */
  error?: string | null;
  /** 削除可能かどうかのマップ（id -> canDelete） */
  canDeleteMap: Record<string, boolean>;
  /** 追加ボタンクリック時のコールバック */
  onAddClick: () => void;
  /** 削除ボタンクリック時のコールバック */
  onDeleteClick: (id: string) => void;
}
```

### 2. コンポーネント実装

```typescript
// packages/web/src/components/deck/MyDeckList.tsx

import { FC } from 'react';
import { MyDeckItem } from './MyDeckItem';
import { Spinner } from '../common/Spinner';
import { Button } from '../common/Button';
import type { MyDeckListProps } from './types';

export const MyDeckList: FC<MyDeckListProps> = ({
  myDecks,
  isLoading,
  error,
  canDeleteMap,
  onAddClick,
  onDeleteClick,
}) => {
  if (isLoading) {
    return (
      <div className="flex justify-center items-center py-8" data-testid="loading-spinner">
        <Spinner size="lg" />
      </div>
    );
  }

  if (error) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-red-700" data-testid="error-message">
        {error}
      </div>
    );
  }

  return (
    <div className="space-y-4">
      {/* ヘッダー */}
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-semibold">使用デッキ一覧</h2>
        <Button
          variant="primary"
          size="sm"
          onClick={onAddClick}
          data-testid="add-deck-button"
        >
          デッキを追加
        </Button>
      </div>

      {/* デッキ一覧 */}
      {myDecks.length === 0 ? (
        <div
          className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center text-gray-500"
          data-testid="empty-message"
        >
          <p className="mb-2">使用デッキが登録されていません</p>
          <p className="text-sm">「デッキを追加」ボタンから新しいデッキを登録してください</p>
        </div>
      ) : (
        <ul className="space-y-2" data-testid="deck-list">
          {myDecks.map((deck) => (
            <li key={deck.id}>
              <MyDeckItem
                deck={deck}
                canDelete={canDeleteMap[deck.id] ?? true}
                onDeleteClick={() => onDeleteClick(deck.id)}
              />
            </li>
          ))}
        </ul>
      )}
    </div>
  );
};
```

### 3. Storybook Story

```typescript
// packages/web/src/components/deck/MyDeckList.stories.tsx

import type { Meta, StoryObj } from '@storybook/react';
import { MyDeckList } from './MyDeckList';

const meta: Meta<typeof MyDeckList> = {
  title: 'Deck/MyDeckList',
  component: MyDeckList,
  tags: ['autodocs'],
};

export default meta;
type Story = StoryObj<typeof MyDeckList>;

const sampleDecks = [
  {
    id: '1',
    deckId: 'deck-001',
    deckName: '秘術オデンスペル',
    deckCode: 'ABC123',
    className: 'ウィッチ',
    isActive: true,
    createdAt: '2025-01-01T00:00:00.000Z',
  },
  {
    id: '2',
    deckId: 'deck-002',
    deckName: 'コントロールネクロ',
    deckCode: 'DEF456',
    className: 'ネクロマンサー',
    isActive: true,
    createdAt: '2025-01-02T00:00:00.000Z',
  },
];

export const Default: Story = {
  args: {
    myDecks: sampleDecks,
    isLoading: false,
    canDeleteMap: { '1': true, '2': false },
    onAddClick: () => console.log('Add clicked'),
    onDeleteClick: (id) => console.log('Delete clicked:', id),
  },
};

export const Loading: Story = {
  args: {
    myDecks: [],
    isLoading: true,
    canDeleteMap: {},
    onAddClick: () => {},
    onDeleteClick: () => {},
  },
};

export const Empty: Story = {
  args: {
    myDecks: [],
    isLoading: false,
    canDeleteMap: {},
    onAddClick: () => console.log('Add clicked'),
    onDeleteClick: () => {},
  },
};

export const WithError: Story = {
  args: {
    myDecks: [],
    isLoading: false,
    error: 'データの取得に失敗しました',
    canDeleteMap: {},
    onAddClick: () => {},
    onDeleteClick: () => {},
  },
};
```

---

## テスト要件

### テストケース一覧

| No | テストケース | 期待結果 | 優先度 |
|----|-------------|----------|--------|
| 1 | 使用デッキが表示されること | デッキ名が表示 | 高 |
| 2 | 0件時に空メッセージが表示されること | 空メッセージ表示 | 高 |
| 3 | ローディング中にスピナーが表示されること | Spinner表示 | 高 |
| 4 | 追加ボタンが表示されること | ボタン存在 | 高 |
| 5 | 追加ボタンクリックでコールバック呼び出し | onAddClick呼び出し | 中 |
| 6 | エラーメッセージが表示されること | エラー表示 | 中 |

### テストコード例

```typescript
// packages/web/src/components/deck/__tests__/MyDeckList.test.tsx

import { describe, it, expect, vi } from 'vitest';
import { render, screen, fireEvent } from '@testing-library/react';
import { MyDeckList } from '../MyDeckList';

const sampleDecks = [
  {
    id: '1',
    deckId: 'deck-001',
    deckName: '秘術オデンスペル',
    deckCode: 'ABC123',
    className: 'ウィッチ',
    isActive: true,
    createdAt: '2025-01-01T00:00:00.000Z',
  },
];

describe('MyDeckList', () => {
  const defaultProps = {
    myDecks: [],
    isLoading: false,
    canDeleteMap: {},
    onAddClick: vi.fn(),
    onDeleteClick: vi.fn(),
  };

  describe('デッキ表示', () => {
    it('使用デッキが表示されること', () => {
      render(
        <MyDeckList
          {...defaultProps}
          myDecks={sampleDecks}
          canDeleteMap={{ '1': true }}
        />
      );

      expect(screen.getByText('秘術オデンスペル')).toBeInTheDocument();
    });

    it('複数のデッキが表示されること', () => {
      const multipleDecks = [
        ...sampleDecks,
        {
          id: '2',
          deckId: 'deck-002',
          deckName: 'コントロールネクロ',
          deckCode: 'DEF456',
          className: 'ネクロマンサー',
          isActive: true,
          createdAt: '2025-01-02T00:00:00.000Z',
        },
      ];

      render(
        <MyDeckList
          {...defaultProps}
          myDecks={multipleDecks}
          canDeleteMap={{ '1': true, '2': true }}
        />
      );

      expect(screen.getByText('秘術オデンスペル')).toBeInTheDocument();
      expect(screen.getByText('コントロールネクロ')).toBeInTheDocument();
    });
  });

  describe('空状態', () => {
    it('0件時に空メッセージが表示されること', () => {
      render(<MyDeckList {...defaultProps} />);

      expect(screen.getByTestId('empty-message')).toBeInTheDocument();
      expect(screen.getByText(/使用デッキが登録されていません/)).toBeInTheDocument();
    });
  });

  describe('ローディング状態', () => {
    it('ローディング中にスピナーが表示されること', () => {
      render(<MyDeckList {...defaultProps} isLoading={true} />);

      expect(screen.getByTestId('loading-spinner')).toBeInTheDocument();
    });

    it('ローディング中はデッキ一覧が表示されないこと', () => {
      render(
        <MyDeckList
          {...defaultProps}
          myDecks={sampleDecks}
          isLoading={true}
        />
      );

      expect(screen.queryByText('秘術オデンスペル')).not.toBeInTheDocument();
    });
  });

  describe('追加ボタン', () => {
    it('追加ボタンが表示されること', () => {
      render(<MyDeckList {...defaultProps} />);

      expect(screen.getByTestId('add-deck-button')).toBeInTheDocument();
    });

    it('追加ボタンクリックでonAddClickが呼び出されること', () => {
      const onAddClick = vi.fn();
      render(<MyDeckList {...defaultProps} onAddClick={onAddClick} />);

      fireEvent.click(screen.getByTestId('add-deck-button'));

      expect(onAddClick).toHaveBeenCalledTimes(1);
    });
  });

  describe('エラー状態', () => {
    it('エラーメッセージが表示されること', () => {
      render(
        <MyDeckList {...defaultProps} error="データの取得に失敗しました" />
      );

      expect(screen.getByTestId('error-message')).toBeInTheDocument();
      expect(screen.getByText('データの取得に失敗しました')).toBeInTheDocument();
    });
  });
});
```

---

## 完了条件

- [ ] MyDeckListコンポーネントが実装されている
- [ ] 使用デッキ一覧が表示される
- [ ] ローディング状態でスピナーが表示される
- [ ] 空リスト時にメッセージが表示される
- [ ] 追加ボタンが配置されている
- [ ] すべてのテストケースが成功する
- [ ] Storybookでコンポーネントが確認できる

---

## 実行コマンド

```bash
# TDDフロー
/tsumiki:tdd-red           # テスト実装（失敗）
/tsumiki:tdd-green         # 最小実装（成功）
/tsumiki:tdd-refactor      # リファクタリング
/tsumiki:tdd-verify-complete  # 品質確認
```

---

## 検証手順

1. テストが失敗することを確認（Red）
2. 最小限の実装でテストが成功することを確認（Green）
3. リファクタリング後もテストが成功することを確認
4. `pnpm test` で全テストが成功することを確認
5. `pnpm storybook` でコンポーネントが正しく表示されることを確認
6. `pnpm lint` でエラーがないことを確認

---

## 参考資料

- [interfaces.ts - MyDeckListProps](../../../docs/design/deck-management-extension/interfaces.ts)
- [React Testing Library](https://testing-library.com/docs/react-testing-library/intro/)
- [Storybook Documentation](https://storybook.js.org/docs)
