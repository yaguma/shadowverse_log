# TASK-0033: 対戦履歴UI改善E2Eテスト

## タスク情報

- **タスクID**: TASK-0033
- **タイプ**: TDD
- **推定工数**: 2時間
- **フェーズ**: Phase 6 - 対戦履歴UI改善
- **信頼性**: 🔵
- **依存タスク**: TASK-0032
- **後続タスク**: TASK-0034
- **ステータス**: [ ] 未完了

---

## 概要

対戦履歴UI改善（レイアウト改善・相手デッキソート）のE2Eテストを作成する。

---

## 実装内容

### 1. E2Eテストファイル作成

`packages/frontend/e2e/battle-log-ui-improvements.spec.ts`:

```typescript
import { test, expect } from '@playwright/test';

test.describe('対戦履歴UI改善', () => {
  test.describe('BattleLogDialog レイアウト改善', () => {
    test.beforeEach(async ({ page }) => {
      await page.goto('/battle-logs');
      await page.click('button:has-text("対戦を記録")');
      await expect(page.locator('[role="dialog"]')).toBeVisible();
    });

    test.describe('デスクトップ表示', () => {
      test.beforeEach(async ({ page }) => {
        await page.setViewportSize({ width: 1024, height: 768 });
      });

      test('シーズンと対戦日が横並びで表示される', async ({ page }) => {
        const seasonLabel = page.locator('label:has-text("シーズン")');
        const dateLabel = page.locator('label:has-text("対戦日")');

        const seasonBox = await seasonLabel.boundingBox();
        const dateBox = await dateLabel.boundingBox();

        expect(seasonBox).not.toBeNull();
        expect(dateBox).not.toBeNull();

        if (seasonBox && dateBox) {
          // Y座標がほぼ同じ（許容誤差10px）
          expect(Math.abs(seasonBox.y - dateBox.y)).toBeLessThan(10);
        }
      });

      test('ターンと勝敗が横並びで表示される', async ({ page }) => {
        const turnLabel = page.locator('label:has-text("ターン")');
        const resultLabel = page.locator('label:has-text("勝敗")');

        const turnBox = await turnLabel.boundingBox();
        const resultBox = await resultLabel.boundingBox();

        expect(turnBox).not.toBeNull();
        expect(resultBox).not.toBeNull();

        if (turnBox && resultBox) {
          expect(Math.abs(turnBox.y - resultBox.y)).toBeLessThan(10);
        }
      });
    });

    test.describe('モバイル表示', () => {
      test.beforeEach(async ({ page }) => {
        await page.setViewportSize({ width: 375, height: 667 });
      });

      test('シーズンと対戦日が縦並びで表示される', async ({ page }) => {
        const seasonLabel = page.locator('label:has-text("シーズン")');
        const dateLabel = page.locator('label:has-text("対戦日")');

        const seasonBox = await seasonLabel.boundingBox();
        const dateBox = await dateLabel.boundingBox();

        expect(seasonBox).not.toBeNull();
        expect(dateBox).not.toBeNull();

        if (seasonBox && dateBox) {
          // 対戦日がシーズンより下に表示される
          expect(dateBox.y).toBeGreaterThan(seasonBox.y + 20);
        }
      });
    });

    test.describe('詳細設定', () => {
      test('詳細設定が折りたたまれて表示される', async ({ page }) => {
        const details = page.locator('details summary');
        await expect(details).toContainText('詳細設定');
      });

      test('詳細設定をクリックすると展開される', async ({ page }) => {
        const details = page.locator('details summary');
        await details.click();

        // ランク選択が表示されることを確認
        await expect(page.locator('label:has-text("ランク")')).toBeVisible();
        await expect(page.locator('label:has-text("グループ")')).toBeVisible();
        await expect(page.locator('label:has-text("対戦種別")')).toBeVisible();
      });
    });
  });

  test.describe('相手デッキ選択肢ソート', () => {
    test.beforeEach(async ({ page }) => {
      // テストデータをセットアップ（対戦履歴を登録してから確認）
      await page.goto('/battle-logs');
    });

    test('相手デッキ選択肢が表示される', async ({ page }) => {
      await page.click('button:has-text("対戦を記録")');
      await expect(page.locator('[role="dialog"]')).toBeVisible();

      const opponentSelect = page.locator('select[name="opponentDeckId"]');
      await expect(opponentSelect).toBeVisible();

      // 選択肢があることを確認
      const options = opponentSelect.locator('option');
      const optionCount = await options.count();
      expect(optionCount).toBeGreaterThan(1); // 「選択してください」+ デッキ選択肢
    });

    test('最近使用したデッキが上位に表示される', async ({ page }) => {
      // 事前条件: すでに対戦履歴が存在し、特定のデッキが使用されている

      await page.click('button:has-text("対戦を記録")');
      await expect(page.locator('[role="dialog"]')).toBeVisible();

      const opponentSelect = page.locator('select[name="opponentDeckId"]');
      const options = opponentSelect.locator('option');

      // 最初のオプション（「選択してください」の次）が最近使用されたデッキ
      // このテストは実際の対戦履歴データに依存する
      const firstDeckOption = options.nth(1);
      const firstDeckText = await firstDeckOption.textContent();

      // 使用回数が表示されている場合（例: "デッキ名 (5回)"）
      // または単にデッキ名のみ
      expect(firstDeckText).toBeTruthy();
    });

    test('対戦履歴がない場合はsortOrder順で表示される', async ({ page }) => {
      // 対戦履歴がない状態（または新しいデッキマスターのみの状態）をシミュレート
      // このテストはテストデータの状態に依存

      await page.click('button:has-text("対戦を記録")');
      await expect(page.locator('[role="dialog"]')).toBeVisible();

      // 選択肢が存在することを確認
      const opponentSelect = page.locator('select[name="opponentDeckId"]');
      await expect(opponentSelect).toBeVisible();
    });
  });

  test.describe('フォーム入力フロー', () => {
    test('レイアウト改善後も正常に対戦履歴を登録できる', async ({ page }) => {
      await page.goto('/battle-logs');

      // 初期の対戦履歴数を記録
      const initialCount = await page.locator('[data-testid="battle-log-row"]').count();

      // 対戦を記録
      await page.click('button:has-text("対戦を記録")');
      await expect(page.locator('[role="dialog"]')).toBeVisible();

      // フォーム入力
      await page.selectOption('select[name="myDeckId"]', { index: 1 });
      await page.selectOption('select[name="opponentDeckId"]', { index: 1 });
      await page.selectOption('select[name="turn"]', '先攻');
      await page.selectOption('select[name="result"]', '勝ち');

      // 保存
      await page.click('button:has-text("保存")');

      // ダイアログが閉じることを確認
      await expect(page.locator('[role="dialog"]')).not.toBeVisible();

      // 対戦履歴が追加されていることを確認
      await page.waitForSelector('[data-testid="battle-log-row"]');
      const newCount = await page.locator('[data-testid="battle-log-row"]').count();
      expect(newCount).toBe(initialCount + 1);
    });
  });
});
```

### 2. テストユーティリティ追加

`packages/frontend/e2e/utils/dialog-helpers.ts`:

```typescript
import { Page, expect } from '@playwright/test';

/**
 * BattleLogDialogを開くヘルパー
 */
export async function openBattleLogDialog(page: Page): Promise<void> {
  await page.click('button:has-text("対戦を記録")');
  await expect(page.locator('[role="dialog"]')).toBeVisible();
}

/**
 * BattleLogDialogを閉じるヘルパー
 */
export async function closeBattleLogDialog(page: Page): Promise<void> {
  await page.click('button:has-text("キャンセル")');
  await expect(page.locator('[role="dialog"]')).not.toBeVisible();
}

/**
 * 要素の位置を比較するヘルパー
 */
export async function compareElementPositions(
  page: Page,
  selector1: string,
  selector2: string
): Promise<{ sameRow: boolean; element1Above: boolean }> {
  const box1 = await page.locator(selector1).boundingBox();
  const box2 = await page.locator(selector2).boundingBox();

  if (!box1 || !box2) {
    throw new Error('要素が見つかりません');
  }

  return {
    sameRow: Math.abs(box1.y - box2.y) < 10,
    element1Above: box1.y < box2.y - 10,
  };
}
```

### 3. レスポンシブテストフィクスチャ

`packages/frontend/e2e/fixtures/responsive-fixtures.ts`:

```typescript
import { test as base } from '@playwright/test';

interface ResponsiveFixtures {
  setDesktopViewport: () => Promise<void>;
  setMobileViewport: () => Promise<void>;
  setTabletViewport: () => Promise<void>;
}

export const test = base.extend<ResponsiveFixtures>({
  setDesktopViewport: async ({ page }, use) => {
    const setDesktop = async () => {
      await page.setViewportSize({ width: 1280, height: 800 });
    };
    await use(setDesktop);
  },

  setMobileViewport: async ({ page }, use) => {
    const setMobile = async () => {
      await page.setViewportSize({ width: 375, height: 667 });
    };
    await use(setMobile);
  },

  setTabletViewport: async ({ page }, use) => {
    const setTablet = async () => {
      await page.setViewportSize({ width: 768, height: 1024 });
    };
    await use(setTablet);
  },
});

export { expect } from '@playwright/test';
```

---

## テスト要件

### E2Eテストケース一覧

| テストケース | 説明 | 期待結果 |
|---|---|---|
| デスクトップ横並び | シーズン・対戦日の配置 | 同じ行に表示される |
| モバイル縦並び | シーズン・対戦日の配置 | 縦に並んで表示される |
| 詳細設定折りたたみ | 詳細設定の初期状態 | 折りたたまれている |
| 詳細設定展開 | 詳細設定クリック後 | ランク・グループ・種別が表示される |
| 相手デッキ選択肢 | 選択肢の表示 | 選択肢が表示される |
| 最近使用ソート | 相手デッキの順序 | 最近使用順で表示される |
| フォーム入力完了 | 登録フロー | 正常に登録できる |

---

## 完了条件

- [ ] E2Eテストファイルが作成されている
- [ ] レイアウト改善テストが成功する
- [ ] レスポンシブテストが成功する
- [ ] 相手デッキソートテストが成功する
- [ ] フォーム入力フローテストが成功する
- [ ] すべてのE2Eテストが成功する

---

## 実行コマンド

```bash
# E2Eテスト実行
pnpm exec playwright test e2e/battle-log-ui-improvements.spec.ts

# UIモードで実行（デバッグ用）
pnpm exec playwright test e2e/battle-log-ui-improvements.spec.ts --ui

# 特定のビューポートサイズでテスト
pnpm exec playwright test e2e/battle-log-ui-improvements.spec.ts --project=Mobile\ Chrome
```

---

## 検証手順

1. `packages/frontend/e2e/battle-log-ui-improvements.spec.ts` が存在するか確認
2. `pnpm exec playwright test e2e/battle-log-ui-improvements.spec.ts` でテストが成功するか確認
3. テストレポートを確認（`playwright-report/index.html`）

---

## 関連ドキュメント

- [TASK-0031: BattleLogDialogレイアウト改善](./TASK-0031.md)
- [TASK-0032: 相手デッキ選択肢ソート機能実装](./TASK-0032.md)
