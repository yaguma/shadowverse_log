import { render, screen } from '@testing-library/react';
import { describe, expect, it } from 'vitest';
import { OverallStats } from './OverallStats';

// 【テストファイル概要】: OverallStats（全体統計表示）コンポーネントの単体テスト
// 【テスト目的】: 全体統計の基本表示、数値フォーマットを検証
// 【テスト範囲】: 正常系（3ケース）
// 🔵 信頼性レベル: REQ-203、TASK-0037要件定義書に基づいた実装
// 【更新内容】: WinRateGaugeを使用したグラフィカル表示に対応

describe('OverallStats', () => {
  // ==================== 1. 正常系テストケース ====================

  describe('正常系テスト', () => {
    it('TC-OVERALL-001: 全体統計が正しく表示される', () => {
      // 【テスト目的】: OverallStatsコンポーネントの基本的な統計表示機能を確認
      // 【テスト内容】: タイトル、勝率、試合数情報が正しく表示されることをテスト
      // 【期待される動作】: WinRateGaugeによりグラフィカルに統計が表示される
      // 🔵 信頼性レベル: REQ-203、受け入れ基準TC-OVERALL-001に基づく

      // ========== Given: テストデータ準備 ==========
      const mockStats = {
        totalGames: 100,
        wins: 60,
        losses: 40,
        winRate: 60.0,
      };

      // ========== When: 実際の処理実行 ==========
      render(<OverallStats stats={mockStats} />);

      // ========== Then: 結果検証 ==========
      // 【検証項目】: タイトル「全体統計」が表示される
      // 🔵 信頼性レベル: REQ-203に基づく
      expect(screen.getByText('全体統計')).toBeInTheDocument();

      // 【検証項目】: 勝率が正しく表示される（小数点第1位まで）
      // 🔵 信頼性レベル: REQ-203に基づく
      expect(screen.getByText('60.0%')).toBeInTheDocument();

      // 【検証項目】: 試合数情報が表示される
      // 🔵 信頼性レベル: REQ-203に基づく
      expect(screen.getByText(/100試合/)).toBeInTheDocument();
      expect(screen.getByText(/60勝/)).toBeInTheDocument();
      expect(screen.getByText(/40敗/)).toBeInTheDocument();
    });

    it('TC-OVERALL-002: 勝率が小数点第1位まで正しく表示される', () => {
      // 【テスト目的】: 勝率の数値フォーマットが仕様通りであることを確認
      // 【テスト内容】: 小数点以下の桁数が第1位まで表示されることをテスト
      // 【期待される動作】: toFixed(1)メソッドにより、常に小数点第1位まで表示される
      // 🔵 信頼性レベル: REQ-203に基づく

      // 【テストデータ準備】: 小数点以下の桁数が多い勝率データを用意
      const mockStats = {
        totalGames: 33,
        wins: 20,
        losses: 13,
        winRate: 60.60606060606061,
      };

      render(<OverallStats stats={mockStats} />);

      // 【検証項目】: 勝率が小数点第1位まで表示される（60.6%）
      // 🔵 信頼性レベル: REQ-203に基づく
      expect(screen.getByText('60.6%')).toBeInTheDocument();
    });

    it('TC-OVERALL-003: カード形式のレイアウトが正しく適用される', () => {
      // 【テスト目的】: グラフィカルUIのレイアウトを確認
      // 【テスト内容】: カード形式のコンテナが正しく適用される
      // 【期待される動作】: bg-white rounded-lg shadow-md のスタイルが適用される
      // 🔵 信頼性レベル: REQ-203、UIデザイン仕様に基づく

      const mockStats = {
        totalGames: 100,
        wins: 60,
        losses: 40,
        winRate: 60.0,
      };

      const { container } = render(<OverallStats stats={mockStats} />);

      // 【検証項目】: カードコンテナが存在する
      // 🔵 信頼性レベル: UIデザイン仕様に基づく
      const cardContainer = container.querySelector('.bg-white.rounded-lg.shadow-md');
      expect(cardContainer).toBeInTheDocument();

      // 【検証項目】: タイトルが正しいスタイルで表示される
      // 🔵 信頼性レベル: UIデザイン仕様に基づく
      const title = screen.getByText('全体統計');
      expect(title.tagName).toBe('H3');
    });
  });
});
